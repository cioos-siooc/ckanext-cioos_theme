...
def get_commands():
    return [sitemap, package_relationships, menu]
...
@sitemap.command()
@click.argument("page_size", default=1000)
@click.argument("max_per_page", default=50000)
def create(page_size, max_per_page):
    ...
    write_sitemap_files(
        count, start, filename_number, file_list, page_size, max_per_page, DIR_SITEMAP
    )
    write_sitemap_files(
        count,
        start,
        ...
        for pkg in pkgs:
            ...
            os.write(
                ...
                (
                    ...
                    % (
                        "%s/dataset/%s%s"
                        % (config.get("ckan.site_url"), pkg.get("name"), url_suffix),
                    )
                ).encode("utf-8"),
            )
            os.write(
...
###########################################################################
@click.group("package-relationships")
...
def rebuild(dataset_id_or_name=None, clear=False):
    ...
    for package_dict in query["results"]:
        ...
        rels_from_schema = get_relationships_from_schema(
            load_json(package_dict.get("aggregation-info", [])), package_dict["name"]
        )
        ...
        try:
            ...
            click.echo(
                "No package relationship found for dataset %s: %r" % package_dict["id"],
                e,
            )
            existing_rels = []
        ...
        if clear:
            ...
            to_delete = to_delete + [
                x
                for x in existing_rels
                if x not in rels_from_schema
                and x["type"]
                not in ["linked_from", "parent_of", "has_derivation", "dependency_of"]
            ]
        ...
        for d in to_delete:
            try:
                ...
                click.echo(
                    "Deleted package relationship %s %s %s"
        ...
        if to_delete:
            ...
            # relationship later
            for r in (
                model.Session.query(model.PackageRelationship)
                .filter(model.PackageRelationship.state == "deleted")
                .all()
            ):
                r.purge()
        ...
        for a in to_add:
            try:
                ...
                click.echo(
                    "Created package relationship %s %s %s"
...
@click.option(
    "--url",
    default=lambda: (
        config.get("ckan.site_home_url", config.get("ckan.site_home"))
        + "/wp-json/ra/menu/"
    ).replace("//", "/"),
    help="[ckan.site_home_url|ckan.site_home]/wp-json/ra/menu/",
)
@click.option(
    "--output", default="/menu/menu_list.html", help="path to menu list output file"
)
@click.option("--echo", default=False, help="echo file output to stdout")
def create(url, output, echo):
    ...
    def process_list_item(menu_item, indent=2, parent_class=""):
        ...
        out += (
            indent_str2
            + '<a href="%s"><span class="mega-indicator">%s</span></a>\n'
    ...
    with open(
        "ckanext-cioos_theme/ckanext/cioos_theme/templates" + output, "w"
    ) as file:
        file.write(content)
