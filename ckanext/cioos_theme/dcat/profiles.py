...
from ckanext.dcat.utils import DCAT_CLEAN_TAGS, DCAT_EXPOSE_SUBCATALOGS, catalog_uri
...
def resource_uri(resource_dict, dataset_dict):
    ...
    if not uri or uri == "None":
        uri = "{0}/resource/{1}".format(
            dataset_uri(dataset_dict).rstrip("/"),
            "ca-cioos-resource_"
            + hashlib.md5(
                (resource_dict["name"] + " " + resource_dict["url"]).encode("utf-8")
            ).hexdigest(),
        )
...
class CIOOSDCATProfile(SchemaOrgProfile):
    def parse_dataset(self, dataset_dict, dataset_ref):
        ...
        dataset_uri = str(dataset_ref) if isinstance(dataset_ref, URIRef) else ""
        ...
        for distribution in self._distributions(dataset_ref):
            ...
            normalize_ckan_format = toolkit.asbool(
                toolkit.config.get("ckanext.dcat.normalize_ckan_format", True)
            )
            ...
            resource_dict["uri"] = (
                str(distribution) if isinstance(distribution, URIRef) else ""
            )
    ...
    def infer_publisher(self, values):
        ...
        if isinstance(load_json(values.get("metadata-point-of-contact")), dict):
            parties = load_json(values.get("cited-responsible-party", "[]")) + [
                (load_json(values.get("metadata-point-of-contact", "{}")))
            ]
        else:
            parties = load_json(
                values.get("cited-responsible-party", "[]")
            ) + load_json(values.get("metadata-point-of-contact", "[]"))

        for responsible_party in parties:
            if "publisher" in responsible_party["role"]:
                name = responsible_party.get(
                    "organisation-name"
                ) or responsible_party.get("individual-name")
                email = responsible_party.get("contact-info_email")
                url = responsible_party.get("contact-info_online-resource_url")
                org_fquri = toolkit.h.cioos_get_fully_qualified_package_uri(
                    responsible_party, "organization-uri"
                )
                if org_fquri:
                    org_fquri = org_fquri[0]
                ind_fquri = toolkit.h.cioos_get_fully_qualified_package_uri(
                    responsible_party, "individual-uri"
                )
                if ind_fquri:
        ...
        if not name:
            ...
            name = toolkit.h.scheming_language_text(
                load_json(org_details.get("title_translated", {}))
            )
            qualified_uri = toolkit.h.cioos_get_fully_qualified_package_uri(
                org_details, "organization-uri"
            )
            if qualified_uri:
                qualified_uri = qualified_uri[0]
            ckan_page_uri = "{0}/organization/{1}".format(
                toolkit.config.get("ckan.site_url").rstrip("/"), org_id
            )
        ...
        for s, p, o in self.g.triples((None, RDF.type, SCHEMA.Dataset)):
            ...
            if isinstance(notes_obj, dict):
                for lang in notes_obj:
                    self.g.add(
                        (s, SCHEMA.description, Literal(notes_obj[lang], lang=lang))
                    )
            else:
        ...
        if inc_dc_list:
            for dc in inc_dc_list:
                ...
                self.g.add(
                    (
                        ...
                        SCHEMA.name,
                        Literal(
                            toolkit.h.scheming_language_text(
                                load_json(dc.get("name", ""))
                            )
                        ),
                    )
                )
            ...
            self.g.add(
                (
                    ...
                    Literal(
                        toolkit.h.scheming_language_text(
                            load_json(toolkit.config.get("ckan.site_title"))
                        )
                    ),
                )
            )
            self.g.add(
                (
                    ...
                    Literal(
                        toolkit.h.scheming_language_text(
                            load_json(toolkit.config.get("ckan.site_description"))
                        )
                    ),
                )
            )
            self.g.add(
                (data_catalog, SCHEMA.url, Literal(toolkit.config.get("ckan.site_url")))
            )
    ...
    def graph_from_dataset(self, dataset_dict, dataset_ref):
        ...
        for mp in load_json(dataset_dict.get("metadata-point-of-contact", "[]")):
            ...
            if name:
                ...
                self.g.add(
                    (contact_details, VCARD.hasEmail, Literal("mailto:" + email))
                )
        ...
        # Creators
        for responsible_party in load_json(
            dataset_dict.get("cited-responsible-party", "[]")
        ):
            if "publisher" in responsible_party["role"]:
                continue
            ...
            if name:
                ...
                self.g.add(
                    (creator_details, SCHEMA.additionalName, Literal(additional))
                )
        ...
        for eov in dataset_dict.get("eov", []):
            ...
            self.g.add(
                (
                    eov_node,
                    SCHEMA.name,
                    Literal(toolkit.h.scheming_choices_label(choices, eov)),
                )
            )
        ...
        if organization:
            ...
            org_uri = (
                fquri
                or organization.get("external_home_url")
                or toolkit.h.url_for(
                    "organization.read", id=organization["name"], _external=True
                )
            )
            ...
            g.add(
                (
                    ...
                    SCHEMA.name,
                    Literal(
                        organization.get("display_name") or organization.get("title")
                    ),
                )
            )
            if organization.get("external_home_url"):
                g.add(
                    (
                        provider_ref,
                        SCHEMA.url,
                        Literal(organization["external_home_url"]),
                    )
                )
        ...
        if unique_identifiers:
            ...
            for unique_identifier in unique_identifiers:
                if "doi.org" in unique_identifier.get(
                    "authority", ""
                ) or not unique_identifier.get("authority"):
                    doi = re.sub(
                        r"^http.*doi\.org/",
                        "",
                    ...
                    if doi and re.match(
                        r"^10.\d{4,9}\/[-._;()/:A-Z0-9]+$", doi, re.IGNORECASE
                    ):
                        identifier = BNode()
                        ...
                        self.g.add(
                            (
                                identifier,
                                SCHEMA.url,
                                Literal("https://doi.org/%s" % doi),
                            )
                        )

                        uri = dataset_uri(dataset_dict)
                        self.g.add(
                            (dataset_ref, DCAT.previousVersion, URIRefOrLiteral(uri))
                        )
                        self.g.add(
                            (dataset_ref, PROV.wasRevisionOf, URIRefOrLiteral(uri))
                        )
        else:
            uri = dataset_uri(dataset_dict)
        ...
        metadata_dates = [
            x.get("value")
            for x in load_json(dataset_dict.get("metadata-reference-date", []))
        ]
            ...
            if start:
                self.g.add(
                    (
                        ...
                        Literal("%s/%s" % (start, end or "..")),
                    )
                )
            elif end:
                self._add_date_triple(
                    dataset_ref, SCHEMA.temporalCoverage, Literal("../%s" % (end))
                )

        # About
        # this section could capture platform or instrument linkages
        acquisition_information = load_json(
            dataset_dict.get("acquisition-information", {})
        )
        if acquisition_information:
            ...
            g.add(
                (
                    instrumentRef,
                    SCHEMA.description,
                    Literal(platform.get("description")),
                )
            )
            ...
            for instrument in instruments:
                ...
                g.add(
                    (
                        instrumentRef,
                        SCHEMA.description,
                        Literal(instrument.get("description")),
                    )
                )
                g.add(
                    (instrumentRef, SCHEMA.url, Literal(instrument.get("identifier")))
                )
